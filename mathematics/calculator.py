...
class Calculator:
    ...
    def __init__(self):
        ...
        self.va_2 = (1 - ((2*938.272046)/200000)**2)    # magnitude of velocity, determined by collision energy
                                                        # (RHIC = 200 MeV) and the proton mass = 938.272046 MeV
    ...
    '''
    ' @Calculate
    ' this function calculate the Magnetic Field of the collision (B)
    ' entries -> xy_list[[x, y]] (float)
    ' exit-> xy[x, y] (float)
    '''
    def calculate_magnetic_field(self, xy_list):
        magnetic_field_x_list = []
        ...
        for index in range(len(magnetic_field_x_list)):
    ...
    '''
    ' @Calculate
    ' this functions represent the equation to calculate the Electric Field of the collision:
    '
    '              ____
    '              \   |                            (1 - va_2) *ra_vector_value
    '   E = a_em *  \    (1)   _______________________________________________________________________ (2)
    '               /          ra_module^3 [ 1 - ( ra_module * square_of(va_2)^2 ) / ra_module^2 ]^3/2
    '              /___|
    '                a
    ' 
    '   where (1) is the sum and are calculate by calculate_electric_field function and call the
    '   __calculate_electric_field_equation function to realise the division (2) (this function)
    '
    ' entries -> ra_vector_value (float), ra_module (float)
    ' exit -> result of the division (2) (float)
    '''
